...
| M13 | seed 데모 시드 | 활성 | 고정 4지역 시드(채우기/비우기) + **지역 시드 생성기**(포커스 지역 Places 검색 → AI 문구 → 종류 선택·수량·반경) + **그룹 관리**(지도 이동·숨김·삭제) + **무대 가져오기**(v2.65 — Persona VC 콘솔 「컨텐츠 채우기」의 JSON `{"nhstage":1,"name","seed"}` 을 포커스 지역의 시드 그룹으로, 골든앵글 나선 결정적 배치, 그룹 메타 `src:'pvc'`) | `SEED_FEED` `SEED_IMG` `SEED_AREAS` `seedFlat` `initDemoSeed` `clearDemoData` · **생성기**(`seedGroups` `sgSearchPlaces` `sgAskAgent` `sgFallbackPlaces` `sgGenerate` `sgCommit` `sgGroupDelete` `sgGroupSetHidden` `renderSeedGroups` `SG_TPL` `SG_THEME`) · **가져오기**(`sgImportStage`) | app.js | v2.65 |
...
- 2026-08-18 M16 (소리): **v2.64.0** — **모든 액션에 어울리는 소리 · 단계마다 켜고 끄기** (콘솔 D196 짝). NH_ACTIONS 41개를 `nhSfxPlay` 까지 추적해 조용하던 16개에 소리를 붙였고, 슬롯을 7→12 로(**heart·coin·ding·whoosh·alarm**, 음원은 기존과 같은 PCM 16bit·모노·22050Hz). 슬롯 열거 네 자리(NH_SFX_KEYS·GAP·DEFAULT·admin 패널 행)를 콘솔 `check:contract` ⑫ 가 센다. **`nhStepMute`** — `nhAct` 가 단계 시작에 심고 `nhSfxPlay` 한 곳에서 거른다(소리는 전부 그 함수를 지난다 — 전수 확인). ⚠️ 검증은 "불렸는가" 가 아니라 **재생됐는가**(`HTMLMediaElement.play` 가로채기)로 — 래퍼가 원본 앞에서 세면 mute 가 안 먹는 것처럼 보인다.
- 2026-08-18 M05+M17: **v2.63.3** — **지도 위 라이브 핀에 `LIVE`**(`.fp-live` — 사진 아래 가운데, 클러스터·점 모드 제외)와 **전환 효과(⏰)를 라벨 위로**(`.dl-onair` 앵커 -20 → **-37px**). ⚠️ 후자는 정지 상태로 재면 겹쳐 보인다 — 0% 프레임이 라벨 자리에서 시작하고 그때 투명도가 0 이다. 음수 `animation-delay`+`paused` 로 진행 지점마다 세워 재야 참값이다.
- 2026-08-18 M05+M10+M16+M17: **v2.63.1** — **안내문은 연하게 · 쿠폰은 버튼에서 · 채택의 순간 · 피드 가로칸 · 라이브 카드 · burst 주제** (사용자 요청 7건 중 앱 6건). ① 타이핑 칸 `::placeholder` 를 아주 연하게(지우지 않는다 — 실제로 쓰는 사람이 있다). ② `couponFly` 가 요소 대신 **재 둔 rect** 도 받는다 — `coupon` 액션은 `nhAfterClose` 에서 받아 그때 `#ds-claim` rect 가 0 이라 화면 가운데에서 날고 있었다. ③ **`rqaSweep`/`rqaBadge`** — 채택의 **상태**(파란 바탕)와 **사건**(빛 훑기 + ✅ 뿅)을 갈랐다. `nhReqRepaint` 뒤에 걸어야 한다. ⑤ `tab` 의 `e` = 피드 가로 칸, **탭 전환보다 먼저** 걸고 `nhReset` 이 되돌린다. ⑥ `nhSanitize` 의 feeds map 이 `kind` 를 안 날라 라이브 카드를 정할 길이 없었다(무대 밝기와 같은 병). ⑦ `nearbySearch` 에 `keyword` — 자리는 이 동네로 두고 주제만 좁힌다. 값은 `n` 한 칸(`nearby:주제`)이라 새 hop 이 없다.
- 2026-08-18 M10+M16: **v2.63.0** — **무대가 까는 지면에도 `사진만`**(`seed.pages[].bare`). v2.62.5 의 `bare` 는 앱 관리자가 올린 지면에만 있었고, 이번엔 콘솔 컨텐츠 탭에서 항목마다 켠다 — 화면 규칙은 같은 `.cps-bare`(글자칸과 그라데이션을 **함께** 접는다). 값이 지나는 네 자리를 다 적었다(콘솔 `PlaySeed.pages`·`toSeed`·`nowhere.ts`·앱 `nhSanitize` 의 pages map) — 마지막이 v2.62.9 에서 무대 밝기를 삼킨 그 종류의 코드다.
- 2026-08-18 M05+M10+M16: **v2.62.9** — **무대 밝기가 앱에 도착조차 못 하고 있었다 · 피드 상단 지면 · 피드 순서 · 근처 실제 장소 사진** (사용자 요청 4건). ① `nhSanitize` 가 seed 를 새 객체로 다시 지으면서 `theme` 을 버리고 있었다(D124 와 같은 병, 다만 무대 칸이라 계약 검사 밖이었다) — 조립과 빈 게이트 양쪽에 넣고, 콘솔 `check:contract` ⑪(무대 칸 대조)을 새로 세워 일부러 빼고 돌려 잡히는 것까지 확인했다. ② `renderNews` 가 `map` 만 보던 것을 `feed` 까지 — 같은 자리가 탭마다 다른 물건이었다(소셜은 제외). ③ **`feedItems[].ord`** — 컨텐츠 탭 표의 ↑↓ 로 피드 순서를 정한다(`ctReorder`). 포커스 구역 우선은 그대로 두고 그 **안에서** 정해진다. ④ **`nhPlacePhotoPrefetch`/`nhBurstPhoto(...,"nearby")`** — burst 실사 사진을 Google Places 장소 사진으로. 회차 시작에 미리 받고, 실패하면 옛 묶음, 출처 표기는 `nhPlaceCredit`(없이 쓰면 약관 위반 — 늦게 도착한 표기를 다시 거는 버그를 잡았다). `sampleTone` 은 Places 주소를 안 잰다(CORS).
- 2026-08-18 M01: **v2.62.8** — 사용자가 만든 **다크용 Map ID**(`5224…3d50`)를 `CONFIG.MAP_ID_DARK` 에 넣었다. 다크에서 `setMapId` 로 갈아타고 덮개는 안 걸린다(실측: 전환마다 딱 한 번 호출·덮개 0). ⚠️ 이 PC 는 키 리퍼러 때문에 지도가 로컬에서 안 떠서 **그림은 배포본에서 확인해야 한다** — 스타일이 실제로 어두운지, ID 를 가는 순간 오버레이가 다시 붙는지.
- 2026-08-18 M01+M09: **v2.62.7** — **지도는 구글의 다크로 간다**. v2.62.6 이 "런타임에 못 바꾼다" 고 적은 것은 **틀렸다** — `styles`(벡터에서 무시)와 `colorScheme`(생성 시 전용)만 보고 **`map.setMapId()` 를 안 봤다**. 로드된 API(v3.65.12f)에 `setMapId` 가 함수로 있고 `ColorScheme` 도 있다(직접 확인). 이제 `CONFIG.MAP_ID_DARK` 가 있으면 테마마다 `setMapId` 로 갈아타고(생성 시에도 그 ID 로 연다) **덮개(`MapTint`)는 그 값이 빌 때만** 걸린다. ID 를 갈면 렌더러가 다시 서므로 뒤이어 한 번 재렌더한다 — 이 앱은 AdvancedMarker·WebGLOverlayView·tilt 사용처가 0건이라 벡터/래스터 어느 쪽도 잃는 것이 없다. 관리자 표시 옵션 › 테마가 지금 어느 길인지 적는다.
