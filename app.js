...
function declutterBoxes(items,cx,cy){
  // items: [{id,ax,ay,w,h,movable}] — movable=말풍선(방향 선택), 그 외=고정 장애물. 반환: {id:dir}
  var placed=[],out={};
  ...
  items.filter(function(it){return it.movable;})
    ...
    .forEach(function(it){
      var want=outwardDir(it.ax-cx,it.ay-cy),best='up',bestScore=Infinity,bestBox=null;
      DECL_DIRS.forEach(function(dir){
        ...
        var score=ov*1000+(dir===want?0:1); // 겹침 최소 우선, 동점이면 바깥 방향
        if(score<bestScore){bestScore=score;best=dir;bestBox=box;}
      });
      out[it.id]=best;placed.push(bestBox);
    });
  return out;
}
...
function declutterOn(m,spots,feeds,reqs){
  ...
  spots.forEach(function(o){
    ...
    items.push({movable:true,id:o.spot.id,ax:o._ax,ay:o._ay,w:r.width||60,h:r.height||24});
    movers.push(o);
  });
