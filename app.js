...
function renderDrawerDemo(){ // 순서: 트렌드존 → 현장 Request → 스팟 (각 블록 상시 표시, 없으면 안내)
  ...
  if(!(typeof fieldRequests!=='undefined'&&fieldRequests.length)){q.body.appendChild(drawerEmpty('등록된 현장 Request가 없어요.'));}
    ...
    fieldRequests.forEach(function(rq){
      var c=document.createElement('div');c.className='rq-card'; // 내부 응답 버튼 때문에 button→div
      ...
      if(rq.answers&&rq.answers.length){
        var a0=rq.answers[0];
        aa.textContent='💬 '+a0.t;
        if(a0.img){var ti=document.createElement('img');ti.className='rqc-thumb';ti.src=a0.img;ti.alt='';c.appendChild(ti);}
      }
      else{aa.textContent='⏳ 응답 대기 중';aa.classList.add('wait');}
      var act=document.createElement('div');act.className='rqc-actions'; // 현장 응답: 코멘트/사진
...
function initPhoneMirror(){
  ...
  phoneMap.addListener('idle',function(){updatePhoneViewportOverlay();updatePhoneLocation();updatePhoneLens();updatePhoneScale();reclusterFeedMarkers();if(currentMode==='trend'&&currentTab==='map'&&zoneCardStyle==='list')renderSummaryZones();});
...
function aiPresetPool(){ // 현재 데모 상황(모드/탭/위치/컨텐츠)에 맞는 질문 후보 — 매번 10개 랜덤 노출
  ...
  var pool=[
    ...
    {q:'지금 사람 많은 곳 피해서 산책하고 싶어',a:'좋아요 밀집이 낮은 조용한 골목길 위주로 안내해 드릴게요. 지도의 한산한 구역을 확인해 보세요.'},
    {q:'오늘 '+loc+' 소식 요약해줘',a:'오늘 '+loc+'에는 새 피드 컨텐츠와 스팟 메시지가 올라왔어요. 요약 지면과 피드 탭에서 한눈에 볼 수 있어요!'},
    {q:'주변 스팟 메시지 요약해줘',a:'주변 이웃들이 남긴 스팟 메시지를 모았어요. 메뉴(햄버거) → 스팟 메시지에서 워드클라우드로 볼 수 있어요!'},
    ...
    {q:'지금 나가면 우산 필요할까?',a:'데모 버전이라 실시간 날씨 연동은 준비 중이에요. 곧 현장 유저의 실시간 답변으로 알려드릴게요! ☔'},
  ...
  if(currentMode==='trend'){
    ...
    pool.push({q:'옆 동네랑 비교하면 어디가 더 활발해?',a:'좋아요·피드 수 기준으로 비교해 보면 지금은 중심 상권 쪽 동이 더 활발해요. 트렌드 모드에서 존 단위로 볼 수 있어요!'});
  }
  ...
  if(typeof fieldRequests!=='undefined'&&fieldRequests.length){
    pool.push({q:'내 Request에 답변 왔는지 확인해줘',a:fieldRequests[0].answers&&fieldRequests[0].answers.length?('📍 '+fieldRequests[0].place+'에 현장 답변이 도착했어요: "'+fieldRequests[0].answers[0].t+'"'):'아직 현장 답변 대기 중이에요. 답변이 오면 바로 알려드릴게요!'});
  }else{
    pool.push({q:'현장 Request는 어떻게 쓰는 거야?',a:'궁금한 위치를 롱프레스하거나 ＋ 메뉴에서 질문을 남기면, 그 현장에 있는 이웃이 실시간으로 답해줘요!'});
  }
  return pool;
}
...
function initAiAgent(mirror){
  ...
  aiBtn.addEventListener('click',function(e){e.stopPropagation();
    ...
    if(panel&&list){
      list.innerHTML='';
      aiRandomPresets(10).forEach(function(p){
...
function showReqBubble(rq){
  ...
  var photo=document.createElement('button');photo.type='button';photo.className='rq-quick rq-direct';photo.textContent='📷 사진 올리기';
  photo.addEventListener('click',function(){answerRequestPhoto(rq.id);});
  ac.appendChild(photo);
...
function answerRequest(id,text,img){ // img: 사진 답변(dataURL, 선택)
  ...
  if(ab){ab.textContent='📍 '+rq.place+' 현장 답변 도착: '+(img?'📷 ':'')+text;ab.classList.add('show');setTimeout(function(){ab.classList.remove('show');},5000);}
}
