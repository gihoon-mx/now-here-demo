...
function canEditSpot(s){ // 관리자 또는 본인이 올린 유저 스팟(라이브 by / 로컬 기기)
  if(currentRole==='admin')return true;
  return !!(s&&s.live&&(!s.by||s.by===myUid()));
}
...
function initFeedThumbClass(){
  ...
  FeedThumb.prototype._canEdit=function(){return currentRole==='admin'||(!!this.item.by&&this.item.by===myUid());}; // 만든이 + 관리자
...
function initSpotComposerClass(){
  ...
  SpotComposer.prototype.commit=function(){
    ...
    else{spot.live=true;spot.by=myUid();demoSpots.push(spot);rebuildSpots();saveLocalSpots();}
  };
...
function liveSpotDoc(sp){return {id:sp.id,lat:sp.lat,lng:sp.lng,text:sp.text||'',emoji:sp.emoji||'💬',color:sp.color||null,by:myUid(),ts:sp.ts||Date.now()};}
...
function saveLocalSpots(){
  try{localStorage.setItem('nowhere_localSpots',JSON.stringify(
    demoSpots.map(function(s){return {id:s.id,lat:s.lat,lng:s.lng,text:s.text,emoji:s.emoji,color:s.color||null,by:s.by||''};})
  ));}catch(e){}
}
function loadLocalSpotsInto(){ // 로컬 폴백 전용 (라이브면 liveSpots 스냅샷이 담당)
  ...
  try{
    ...
    demoSpots=arr.map(function(s){return {id:s.id,lat:s.lat,lng:s.lng,text:s.text||'',emoji:s.emoji||'💬',color:s.color||null,by:s.by||'',live:true};});
  }catch(e){}
}
...
function liveOn(){
  ...
  liveUnsub.feed=fbDb.collection('liveFeed').orderBy('ts','desc').limit(48).onSnapshot(function(snap){
    feedItems=[];snap.forEach(function(dc){var v=dc.data();feedItems.push({id:dc.id,type:'photo',src:v.src,region:v.region||'',zone:v.zone||null,lat:(v.lat!=null?v.lat:null),lng:(v.lng!=null?v.lng:null),kind:v.kind||'post',desc:v.desc||'',name:v.name||'',by:v.by||'',ts:v.ts||0,likes:v.likes||{}});});
  ...
  liveUnsub.spots=fbDb.collection('liveSpots').orderBy('ts','desc').limit(120).onSnapshot(function(snap){
    demoSpots=[];snap.forEach(function(dc){var v=dc.data();demoSpots.push({id:dc.id,lat:v.lat,lng:v.lng,text:v.text||'',emoji:v.emoji||'💬',color:v.color||null,by:v.by||'',live:true});});
    rebuildSpots();
...
function feedAdd(src,region,zone,lat,lng,kind,desc){ // 피드 컨텐츠 추가 (라이브=공유 / 로컬=이 기기) — kind: 'cam'(라이브 카메라)|'post'(Feed 작성/업로드)
  ...
  var doc={src:src,region:region||'',zone:zone||null,lat:(lat!=null?lat:null),lng:(lng!=null?lng:null),kind:kind||'post',desc:(desc||'').slice(0,120),name:chatName(),by:myUid(),ts:Date.now(),likes:{}};
...
function allFeedEntries(){ // 라이브 사진 + 스팟 + 동네소식 → 포커스 구역 우선 정렬
  var arr=[];
  feedItems.forEach(function(f){var pc=feedItemLatLng(f);arr.push({id:f.id,type:'photo',src:f.src,region:f.region||'',zone:f.zone||null,kind:f.kind||'post',desc:f.desc||'',name:f.name||'',by:f.by||'',ts:f.ts||0,lat:pc?pc.lat:null,lng:pc?pc.lng:null});});
...
function renderFeed(){
  ...
  arr.forEach(function(it){
    ...
    var mine=srcItem&&(currentRole==='admin'||(!!srcItem.by&&srcItem.by===myUid()));
