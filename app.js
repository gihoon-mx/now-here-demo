...
function initSpotComposerClass(){
  ...
  SpotComposer.prototype.commit=function(){
    ...
    else{spot.live=true;demoSpots.push(spot);rebuildSpots();saveLocalSpots();}
  };
...
function saveLocalSpots(){
  try{localStorage.setItem('nowhere_localSpots',JSON.stringify(
    demoSpots.map(function(s){return {id:s.id,lat:s.lat,lng:s.lng,text:s.text,emoji:s.emoji,color:s.color||null};})
  ));}catch(e){}
}
function loadLocalSpotsInto(){ // 로컬 폴백 전용 (라이브면 liveSpots 스냅샷이 담당)
  ...
  try{
    ...
    demoSpots=arr.map(function(s){return {id:s.id,lat:s.lat,lng:s.lng,text:s.text||'',emoji:s.emoji||'💬',color:s.color||null,live:true};});
  }catch(e){}
}
...
function renderDrawerDemo(){ // 순서: 트렌드존 → 현장 Request → 스팟 (각 블록 상시 표시, 없으면 안내)
  ...
  // ③ 스팟 메시지: 말풍선 · 세로
  var sp=dsSection('spots','💬 스팟 메시지');
  if(!spotMessages.length){sp.body.appendChild(drawerEmpty('등록된 스팟 메시지가 없어요.'));}
  else{
    var list=document.createElement('div');list.className='sp-list';
    spotMessages.forEach(function(m){
      var b=document.createElement('button');b.type='button';b.className='sp-bub';
      b.innerHTML='<span class="spb-emoji"></span><span class="spb-text"></span>';
      b.querySelector('.spb-emoji').textContent=m.emoji||'💬';
      b.querySelector('.spb-text').textContent=(m.text||'').trim()||'(빈 메시지)';
      b.style.background=hexToRgba(m.color||spotConfig.bgColor||'#1c66e5',1);
...
function allFeedEntries(){ // 라이브 사진 + 스팟 + 동네소식 → 포커스 구역 우선 정렬
  var arr=[];
  feedItems.forEach(function(f){var pc=feedItemLatLng(f);arr.push({id:f.id,type:'photo',src:f.src,region:f.region||'',zone:f.zone||null,kind:f.kind||'post',desc:f.desc||'',name:f.name||'',ts:f.ts||0,lat:pc?pc.lat:null,lng:pc?pc.lng:null});});
...
function renderFeed(){
  ...
  arr.forEach(function(it){
    ...
    if(tl){var tm=document.createElement('span');tm.className='fc-time';tm.textContent=tl;c.appendChild(tm);}
    var L=likeInfo(it.id);
...
var FEATURES=[
 ...
 {id:'spot',icon:'💬',name:'스팟 메시지',st:'live',grp:'컨텐츠',desc:'지도 위 말풍선 일상 공유 — 관리자+유저 모두 작성, 유저 스팟은 계정 간 실시간 공유(liveSpots). 렌즈 포커스 밖은 옅게.',rel:['lens','feed'],prev:'spot'},
