...
function initSpotBubbleClass(){
  ...
  // 편집 권한자(관리자·본인): 드래그로 이동 / (이동 없이) 클릭하면 편집 모달
  SpotBubble.prototype._onDown=function(e){
    ...
    var moved=false,sx=e.clientX,sy=e.clientY,mapEl=m.getDiv();
    ...
    function up(){
      self.div.removeEventListener('pointermove',mv);self.div.removeEventListener('pointerup',up);self.div.removeEventListener('pointercancel',up);
      m.setOptions({draggable:prevDrag!==false});if(self.div)self.div.classList.remove('dragging');
      if(moved){renderSpots();persistSpotEdit(self.spot);}else{openSpotEditor(self.spot.id);} // 저장: 라이브=liveSpots·로컬=기기 / 관리자=클라우드
    }
    self.div.addEventListener('pointermove',mv);
    self.div.addEventListener('pointerup',up);
    self.div.addEventListener('pointercancel',up);
  };
...
function initFeedThumbClass(){
  ...
  FeedThumb.prototype._canEdit=function(){return currentRole==='admin'||ownsContent(this.item);}; // 만든이(uid/이메일) + 관리자
  FeedThumb.prototype.onAdd=function(){
    ...
    this.div=d;
    if(this._canEdit()){ // 드래그로 위치 이동 가능 → 인터랙티브 pane
  ...
  FeedThumb.prototype._onDown=function(e){ // 드래그 = 컨텐츠 위치 이동 (이동 시 동/존 자동 재태깅)
    var self=this,m=self.getMap();if(!m)return;
    e.stopPropagation();if(e.cancelable)e.preventDefault();
    var mapEl=m.getDiv(),moved=false,sx=e.clientX,sy=e.clientY;
    var prevDrag=m.get('draggable');
    m.setOptions({draggable:false});
    self.div.classList.add('dragging');
    try{self.div.setPointerCapture(e.pointerId);}catch(_){}
    ...
    function up(){
      self.div.removeEventListener('pointermove',mv);self.div.removeEventListener('pointerup',up);self.div.removeEventListener('pointercancel',up);
      m.setOptions({draggable:prevDrag!==false});
      if(self.div)self.div.classList.remove('dragging');
      if(!moved)return;
...
function dsSection(key,title){ // 접이식 드로어 섹션
  ...
  head.innerHTML='<span class="ds-tt"></span><i class="ds-chev">▾</i>';
...
function renderDrawerDemo(){ // 순서: 트렌드존 → 현장 Request → 스팟 (각 블록 상시 표시, 없으면 안내)
  ...
  if(!(typeof fieldRequests!=='undefined'&&fieldRequests.length)){q.body.appendChild(drawerEmpty('등록된 현장 Request가 없어요.'));}
  else{
    var qs=document.createElement('div');qs.className='rq-scroll';
    fieldRequests.forEach(function(rq){
      var c=document.createElement('button');c.type='button';c.className='rq-card';
      c.innerHTML='<span class="rqc-place"></span><span class="rqc-q"></span><span class="rqc-a"></span>';
      c.querySelector('.rqc-place').textContent='📍 '+rq.place;
      c.querySelector('.rqc-q').textContent='"'+rq.q+'"';
      var aa=c.querySelector('.rqc-a');
      if(rq.answers&&rq.answers.length)aa.textContent='💬 '+rq.answers[0].t;
      else{aa.textContent='⏳ 응답 대기 중';aa.classList.add('wait');}
  ...
  var sp=dsSection('spots','💬 스팟 메시지'+(focName?' · '+focName:''));
...
function openRequestComposer(){
  ...
  var rq={id:'rq_'+Date.now(),lat:ll.lat(),lng:ll.lng(),q:q.trim(),place:d?d.name:'지정 위치',answers:[],ts:Date.now()};
  ...
  if(ab){ab.textContent='📍 Request 전송! 현장 유저에게 알림이 갑니다…';ab.classList.add('show');setTimeout(function(){ab.classList.remove('show');},2200);}
  setTimeout(function(){showReqBubble(rq);},2600); // 데모: 현장 유저 수신 알림 시뮬레이션
}
function showReqBubble(rq){
  ...
  ['지금 한산해요','조금 붐벼요','대기 20분+'].forEach(function(t){
    var btn=document.createElement('button');btn.type='button';btn.className='rq-quick';btn.textContent=t;
    btn.addEventListener('click',function(){answerRequest(rq.id,t);});
    ac.appendChild(btn);
  });
  var direct=document.createElement('button');direct.type='button';direct.className='rq-quick rq-direct';direct.textContent='✍ 직접 답장';
  direct.addEventListener('click',function(){var t=prompt('현장 답변을 입력하세요');if(t&&t.trim())answerRequest(rq.id,t.trim());});
  ac.appendChild(direct);
...
function answerRequest(id,text){
  var rq=fieldRequests.find(function(r){return r.id===id;});if(!rq)return;
  if(hasLive()){fbDb.collection('liveRequests').doc(id).update({answers:firebase.firestore.FieldValue.arrayUnion({t:text,ts:Date.now()})});}
  else{rq.answers.push({t:text,ts:Date.now()});saveRequests();renderRequestMarkers();}
  hideReqBubble();
  var ab=document.getElementById('ai-bubble'); // 요청자에게 답변 도착 (데모)
  if(ab){ab.textContent='📍 '+rq.place+' 현장 답변 도착: '+text;ab.classList.add('show');setTimeout(function(){ab.classList.remove('show');},5000);}
}
function renderRequestMarkers(){
  ...
  if(typeof renderDrawerDemo==='function')renderDrawerDemo();
  fieldRequests.forEach(function(rq){
    var lb=new MapLabel(new google.maps.LatLng(rq.lat,rq.lng),(rq.answers.length?'💬 '+rq.answers[0].t:'❓ Request'),{bg:'rgba(32,36,44,0.9)',color:'#fff',fontSize:10},phoneMap);
    reqMarkers.push(lb);
  });
}
...
var SEED_ZONES=[
 {id:'tzs_food',name:'강남 먹자골목',color:'#ff5e7e',theme:'food',img:'gwangjang',desc:'퇴근 후 웨이팅 전쟁, 강남역 골목 맛집 라인',lat:37.4991,lng:127.0292},
 {id:'tzs_cafe',name:'역삼 카페로드',color:'#a9764f',theme:'cafe',img:'garosu',desc:'조용한 골목 스페셜티 카페 산책 코스',lat:37.5008,lng:127.0349},
 {id:'tzs_park',name:'학동공원 러닝코스',color:'#2f9d6f',theme:'park',img:'parkMay',desc:'아침 러닝·저녁 산책, 동네 반려견 성지',lat:37.5147,lng:127.0300}];
var SEED_FEED=[
 {theme:'food',img:'gopchang',label:'수요미식회 그 집',desc:'웨이팅 40분인데 후회 없음. 곱창은 진리',kind:'cam',region:'역삼1동',zone:'tzs_food',lat:37.4988,lng:127.0295,likes:14,h:2,name:'퇴근길미식가'},
 {theme:'food',img:'kfood8',label:'점심 특선',desc:'강남 직장인 점심 1만원 이하 몇 없는 집',kind:'post',region:'역삼1동',zone:'tzs_food',lat:37.4995,lng:127.0288,likes:9,h:5,name:'강남11년차'},
 {theme:'cafe',img:'latte',label:'골목 안 로스터리',desc:'원두 직접 볶는 집. 라떼아트 미쳤다',kind:'cam',region:'역삼1동',zone:'tzs_cafe',lat:37.5010,lng:127.0352,likes:12,h:3,name:'카페투어러'},
 {theme:'cafe',img:'cafeInt',label:'창가 자리 맛집',desc:'노트북 작업하기 좋아요. 콘센트 넉넉',kind:'post',region:'역삼1동',zone:'tzs_cafe',lat:37.5004,lng:127.0344,likes:7,h:8,name:'프리랜서J'},
 {theme:'park',img:'parkPath',label:'아침 러닝',desc:'오늘 학동공원 5km 완주. 공기 최고',kind:'cam',region:'논현1동',zone:'tzs_park',lat:37.5149,lng:127.0297,likes:11,h:1,name:'러닝크루장'},
 {theme:'pet',img:'dog',label:'댕댕이 산책',desc:'공원에서 만난 리트리버. 순둥이 그 자체',kind:'cam',region:'논현1동',zone:'tzs_park',lat:37.5143,lng:127.0306,likes:16,h:4,name:'산책하는댕댕이'},
 {theme:'night',img:'pojang',label:'심야 포차',desc:'새벽 2시에도 자리 없는 그 포차',kind:'cam',region:'논현1동',zone:null,lat:37.5108,lng:127.0224,likes:8,h:26,name:'야식원정대'},
 {theme:'shop',img:'gangnam',label:'팝업 오픈',desc:'신논현 팝업스토어 오늘 오픈! 줄 김',kind:'cam',region:'논현1동',zone:null,lat:37.5047,lng:127.0248,likes:10,h:6,name:'트렌드헌터'},
 {theme:'gym',img:'gym',label:'새벽 운동',desc:'오운완. 6시 헬스장은 평화롭다',kind:'post',region:'역삼1동',zone:null,lat:37.5001,lng:127.0371,likes:5,h:7,name:'갓생살기'},
 {theme:'book',img:'book',label:'동네 책방',desc:'논현동에 이런 독립서점이 있었다니',kind:'post',region:'논현1동',zone:null,lat:37.5118,lng:127.0243,likes:6,h:30,name:'책읽는밤'},
 {theme:'food',img:'brunch',label:'브런치 신상',desc:'주말 브런치 신상 오픈. 프렌치토스트 추천',kind:'post',region:'논현2동',zone:null,lat:37.5165,lng:127.0355,likes:9,h:20,name:'주말미식'},
 {theme:'cafe',img:'cheesecake',label:'디저트 맛집',desc:'치즈케이크 마감 전에 가세요',kind:'cam',region:'역삼2동',zone:null,lat:37.4967,lng:127.0446,likes:13,h:9,name:'디저트지도'},
 {theme:'art',img:null,label:'골목 벽화',desc:'출근길에 발견한 새 벽화. 사진각',kind:'cam',region:'역삼2동',zone:null,lat:37.4959,lng:127.0421,likes:4,h:11,name:'골목산책자'},
 {theme:'run',img:'seokchonLake',label:'퇴근 러닝',desc:'호수 한 바퀴 야간 러닝 함께해요 (매주 화)',kind:'post',region:'역삼1동',zone:null,lat:37.5017,lng:127.0392,likes:7,h:28,name:'러닝크루장'},
 {theme:'shop',img:'flea3',label:'플리마켓',desc:'이번 주말 학동공원 플리마켓 열려요',kind:'post',region:'논현1동',zone:'tzs_park',lat:37.5151,lng:127.0291,likes:8,h:14,name:'동네소식통'},
 {theme:'night',img:'rooftop',label:'루프탑',desc:'강남 야경 루프탑. 예약 필수',kind:'cam',region:'역삼1동',zone:'tzs_food',lat:37.4998,lng:127.0301,likes:15,h:50,name:'야경수집가'},
 {theme:'food',img:'noodle',label:'칼국수 맛집',desc:'점심 칼국수 오픈런 성공. 육수가 예술',kind:'cam',region:'역삼1동',zone:'tzs_food',lat:37.4992,lng:127.0298,likes:6,h:4,name:'점심원정대'},
 {theme:'cafe',img:'espresso',label:'에스프레소 바',desc:'서서 마시는 에바. 2잔이 국룰입니다',kind:'post',region:'역삼1동',zone:'tzs_cafe',lat:37.5013,lng:127.0346,likes:10,h:12,name:'카페투어러'},
 {theme:'food',img:'kfood9',label:'전집 발견',desc:'비 오는 날 전+막걸리 조합 아시죠',kind:'cam',region:'논현1동',zone:'tzs_food',lat:37.4993,lng:127.0284,likes:12,h:16,name:'막걸리동호회'},
 {theme:'park',img:'cherry',label:'벚꽃 스팟',desc:'벚꽃 마지막 주라는데 오늘이 절정',kind:'cam',region:'논현1동',zone:'tzs_park',lat:37.5146,lng:127.0303,likes:18,h:3,name:'꽃놀이객'}];
var SEED_SPOTS=[
 ...
 {t:'칼국수 오픈런 성공',emoji:'🍲',lat:37.4992,lng:127.0297,color:'#ff5e7e'},
 {t:'라떼아트 클래스 모집 중',emoji:'🎨',lat:37.5006,lng:127.0341,color:'#a9764f'},
 {t:'벚꽃 포토스팟은 여기',emoji:'📸',lat:37.5148,lng:127.0294,color:'#f78fb3'},
 {t:'불금 번개 8시 어떠세요',emoji:'🍻',lat:37.4990,lng:127.0286}];
var SEED_REQS=[
...
function seedDemoData(){
  ...
  if(!confirm('강남·역삼·논현 데모 데이터를 채울까요?\n(트렌드 존 3 · 피드 20 · 스팟 16 · Request 2 · 채팅 시드 — 공유 컬렉션에 기록되어 모든 계정에 보여요.\n컨텐츠 소유자: '+SEED_OWNER+')'))return;
  var now=Date.now();
  // ① 트렌드 존 (id 중복 방지)
  SEED_ZONES.forEach(function(sz){
    if(trendZones.some(function(z){return z.id===sz.id;}))return;
    var hc=seedHexCluster(sz.lat,sz.lng);
    trendZones.push({id:sz.id,name:sz.name,color:sz.color,desc:sz.desc,photo:(sz.img?SEED_IMG[sz.img]:seedImg(sz.theme,sz.name)),
...
var FEATURES=[
 ...
 {id:'spot',icon:'💬',name:'스팟 메시지',st:'live',grp:'컨텐츠',desc:'지도 위 말풍선 일상 공유 — 관리자+유저 모두 작성, 유저 스팟은 계정 간 실시간 공유(liveSpots). 본인이 올린 스팟은 데모도 드래그 이동·수정·삭제 가능. 드로어=현재 지역 워드 클라우드. 렌즈 포커스 밖은 옅게.',rel:['lens','feed'],prev:'spot'},
 ...
 {id:'feed',icon:'🖼️',name:'피드 탭',st:'live',grp:'서비스 탭',desc:'그리드 피드(1:1, 열 1~3·핀치·간격) — 컨텐츠 속성: 종류(라이브/피드작성)·만든이·위치·존·설명·좋아요·올린시간(상대/시각 옵션). 범위 필터가 모드를 따라감. 카드=존 칩·LIVE 칩·시간, 탭=지도에서 위치 보기. 지도 핀은 만든이·관리자가 드래그 이동(동/존 자동 재태깅). 지역 컨텐츠 지면에 연관 피드 자동 노출(스팟 제외).',rel:['cam','like','mode','sum']},
