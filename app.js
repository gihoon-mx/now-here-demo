...
function renderDrawerDemo(){ // 순서: 트렌드존 → 현장 Request → 스팟 (각 블록 상시 표시, 없으면 안내)
  ...
  if(!(typeof fieldRequests!=='undefined'&&fieldRequests.length)){q.body.appendChild(drawerEmpty('등록된 현장 Request가 없어요.'));}
    ...
    fieldRequests.forEach(function(rq){
      ...
      c.innerHTML='<span class="rqc-place"></span><span class="rqc-q"></span>'; // 응답 대기/결과는 표시 안 함(요청자에게만 실시간 도착 알림)
...
function liveOn(){
  ...
  liveUnsub.reqs=fbDb.collection('liveRequests').orderBy('ts','desc').limit(40).onSnapshot(function(snap){
    ...
    fieldRequests=[];snap.forEach(function(dc){var v=dc.data();fieldRequests.push({id:dc.id,lat:v.lat,lng:v.lng,q:v.q,place:v.place,answers:v.answers||[],by:v.by||'',ts:v.ts||0});});
    renderRequestMarkers();
    ...
    changes.forEach(function(ch){
      ...
      if(ch.type==='added'){
        ...
        // 새 Request → 타겟 지역(내가 보고 있는 근처) 사용자에게 AI Agent 응답 팝업 (내 것·오래된 것 제외)
        if(v.by!==myUid()&&Date.now()-(v.ts||0)<3*60000&&reqNearMe(v))showReqBubble({id:id,q:v.q,place:v.place,lat:v.lat,lng:v.lng});
...
function renderRequestMarkers(){
  ...
  if(typeof renderDrawerDemo==='function')renderDrawerDemo();
  fieldRequests.forEach(function(rq){ // 답변 내용은 지도에 노출 안 함(요청자에게만 도착 알림)
    var lb=new MapLabel(new google.maps.LatLng(rq.lat,rq.lng),'❓ Request',{bg:'rgba(32,36,44,0.9)',color:'#fff',fontSize:10},phoneMap);
    reqMarkers.push(lb);
  });
}
...
var FEATURES=[
 ...
 {id:'req',icon:'📍',name:'현장 Request',st:'live',grp:'컨텐츠',desc:'원격 질문 등록 → 타겟 지역(1.5km/같은 동) 사용자에게 AI Agent 실시간 응답 팝업(💬 답하기·📷 사진). 답변은 요청자에게만 도착 알림(liveRequests).',rel:['map','ai']},
